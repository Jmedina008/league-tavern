...
interface DebugChatPageProps {
  params: Promise<{ subdomain: string }>
}

export default function DebugChatPage({ params }: DebugChatPageProps) {
        ...
        <div className="max-w-md mx-auto px-4 py-16">
          <Card className="bg-slate-800 border-slate-700">
            <CardHeader className="text-center">
              ...
              <Badge className="bg-yellow-600 text-white mt-2">DEBUG MODE</Badge>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="text-center">
                <Link href={`/debug/league/${subdomain}`} className="text-blue-400 hover:text-blue-300 text-sm">
                  ← Back to league home
                </Link>
              </div>
            </CardContent>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-400">
              Signed in as {currentUser?.sleeperUsername || currentUser?.ownerName} 
              <Badge className="bg-yellow-600 text-white ml-2 text-xs">DEBUG</Badge>
            </p>
          </div>
          <div className="flex items-center space-x-2">
            <Link 
              href={`/debug/league/${subdomain}`}
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      {onlineUsers.length === 0 && (
                        <div className="text-slate-400 text-sm">No one online yet</div>
                      )}
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card className="bg-slate-800 border-slate-700">
                ...
                <CardContent>
                  {forumThreads.length === 0 ? (
                    <div className="text-center text-slate-400 py-8">
                      <p className="mb-4">No forum threads yet.</p>
                      <Button 
                        onClick={async () => {
                          try {
                            ...
                            if (response.ok) {
                              // Refresh threads
                              const threadsData = await fetch(`/api/forum/${subdomain}/threads`).then(r => r.json())
                              if (threadsData.threads) setForumThreads(threadsData.threads)
                            }
                          } catch (error) {
                      ...
                      >
                        Initialize Forum Threads
                      </Button>
                    </div>
                  ) : (
                    <div className="space-y-3">
                      {forumThreads
                        .filter(thread => selectedCategory === 'all' || thread.category.toLowerCase().replace('_', '-') === selectedCategory)
                        .map((thread) => (
